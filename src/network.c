...
void forward_network(network net, network_state state)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        // 显示卷积后的结果图像
        //if ((l.type == 0 || l.type == 3 || l.type==28)) {
        if (l.type==28) {
            printf("the %d layer, layer type: %d, layer size: [%d,%d,%d]\n", i + 1, l.type, l.w, l.h, l.c);
            image imgs;           
            char s[50];
            if (l.type == 0) {
                imgs = get_convolutional_image(l);
                sprintf(s, "conv_channel_%d\n", i + 1);
            }
            else if (l.type==3) {
                imgs = get_maxpool_image(l);
                sprintf(s, "maxpool_channel_%d\n", i + 1);
            }
            else {
                imgs = float_to_image(l.w, l.h, l.c, l.output);
                sprintf(s, "yolo_layer_%d", (i + 1));
            }

            char ss[50];
            for (int k = 0; k < 10; k++) {
                image c = get_image_layer(imgs, k);  // 显示指定通道
                sprintf(ss, "%s_%d", s, k + 1);
                //show_image(c, ss);

                print_image(c);
            }

            //show_image_layers(imgs,s); // 显示多个通道
            //wait_key_cv(0);
        }
