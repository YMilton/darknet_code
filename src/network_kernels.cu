...
void forward_network_gpu(network net, network_state state)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        if (l.type == 0 || l.type==3) {
            printf("the %d layer, layer type: %d", i + 1, l.type);
            image img = get_convolutional_image(l);
            image c = get_image_layer(img, 6);
            // show_image_layers(); // 显示多个通道
            show_image(c, "conv_channel");
        }
