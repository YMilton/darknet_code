    ...
    for(f = 0; f < n; ++f){
        ...
        for(b = 0; b < batch; ++b){
            for(i = 0; i < size; ++i){
                ...
                sum += delta[index] * x_norm[index]; // delta[index]=d(L)/d(yi)
            }
        }
        scale_updates[f] += sum;
    }
    ...
    for(i = 0; i < filters; ++i){
        ...
        for (j = 0; j < batch; ++j) {
            for (k = 0; k < spatial; ++k) {
                ...
                mean_delta[i] += delta[index]; // delta[index]: dL/dx_norm
            }
        }
        mean_delta[i] *= (-1./sqrt(variance[i] + .00001f)); // 丢弃了后半部分：dL/d(sigmaB^2)*1/m*{sum_i^m}(-2(x_i - uB))
    }
}
// 针对方差var=(sigmaB)^2求导
void  variance_delta_cpu(float *x, float *delta, float *mean, float *variance, int batch, int filters, int spatial, float *variance_delta)
{
    ...
    for(i = 0; i < filters; ++i){
        ...
        for(j = 0; j < batch; ++j){
            for(k = 0; k < spatial; ++k){
                int index = j*filters*spatial + i*spatial + k;
                variance_delta[i] += delta[index]*(x[index] - mean[i]); // delta[index]: dL/dx_norm
            }
        }
    ...
    for(j = 0; j < batch; ++j){
        for(f = 0; f < filters; ++f){
            for(k = 0; k < spatial; ++k){
                ...
                delta[index] = delta[index] * 1./(sqrt(variance[f]) + .00001f) // delta[index]: dL/dx_norm
                    + variance_delta[f] * 2. * (x[index] - mean[f]) / (spatial * batch) // variance_delta[f]: dL/d(sigmaB)^2
                    + mean_delta[f]/(spatial*batch); // mean_delta[f]: dL/d(uB)
            }
        }
    }
}
...
void forward_batchnorm_layer(layer l, network_state state)
{
    ...
    if(state.train){
        mean_cpu(l.output, l.batch, l.out_c, l.out_h*l.out_w, l.mean); // 均值
        variance_cpu(l.output, l.mean, l.batch, l.out_c, l.out_h*l.out_w, l.variance); // 方差
        // 滑动均值与方差
        ...
        copy_cpu(l.outputs*l.batch, l.output, 1, l.x, 1); // 数据复制
        normalize_cpu(l.output, l.mean, l.variance, l.batch, l.out_c, l.out_h*l.out_w); // 归一化
